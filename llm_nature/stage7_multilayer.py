...
def run(ctx: RunContext) -> Dict[str, Any]:
    ...
    layers_env = os.environ.get("LLM_NATURE_PATCH_LAYERS", "9,10,11")
    ...
    clean_vecs: Dict[int, torch.Tensor] = {}
    ...
    for layer in layers:
        ...
        def grab(module: Any, inp: Any, out: Any) -> None:
            ...
            v = hs[:, pos, :].detach().clone()

        h = model.transformer.h[layer].register_forward_hook(grab)
        _run_logits(model, tok, clean, device)
        h.remove()
        if v is None:
            raise RuntimeError(f"failed to capture clean vec at layer {layer}")
        clean_vecs[layer] = v
    ...
    hooks = []
    for layer in layers:
        hooks.append(model.transformer.h[layer].register_forward_hook(PatchHook(clean_vecs[layer], pos)))
    ...
    lines: List[str] = []
    lines.append("=" * 80)
    lines.append("EXPERIMENT 7 MULTILAYER PATCH")
    lines.append("=" * 80)
    lines.append("")
    lines.append(f"Device: {device}")
    lines.append(f"Model: {model_name}")
    lines.append(f"Patch layers: {layers}")
    ...
    return {"stage": 7, "kind": "multilayer", "patch_layers": layers}
