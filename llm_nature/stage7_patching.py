...
def _block_out_replace(out, new_t):
    if isinstance(out, tuple):
        outl = list(out)
        for i, x in enumerate(outl):
            try:
                if torch.is_tensor(x) or hasattr(x, "shape"):
                    outl[i] = new_t
                    return tuple(outl)
            except Exception:
                pass
...
def capture_block_output(model, tok, prompt: str, layer: int, position: int, device: str):
    ...
    def hook(module, inp, out):
        t = _block_out_tensor(out)
        cache["vec"] = t[0, position, :].detach().clone()
...
class ResidualPatcher:
    ...
    def _hook(self, module, inp, out):
        t = _block_out_tensor(out).clone()
        t[0, self.position, :] = self.vec.to(t.device)
        return _block_out_replace(out, t)
...
def run(ctx: RunContext) -> dict:
    ...
    lines = []
    lines.append(banner("EXPERIMENT 7: ACTIVATION PATCHING — GPT-2 BRAIN SURGERY"))
